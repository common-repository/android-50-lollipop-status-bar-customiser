...
Description: Use this plugin to change the Status Bar of Devices Running Android 5.0+
Author: Niraj Shah
Version: 2.1
...
class WN_Android_50_Statusbar
{
  ...
  public function add_android_meta()
  {
    ...
    if ( is_front_page() ) {
      ...
      if ( isset( $cats[0] ) && isset( $this->options['cat_'.$cats[0]->cat_ID] ) && !empty( $this->options['cat_'.$cats[0]->cat_ID] ) ) {
        ...
        $output .= '<meta name="theme-color" content="#'. $this->options['cat_'.$cats[0]->cat_ID] .'">' . PHP_EOL;
      } else {
        // default post colour
  ...
  {
    printf(
      '<input type="checkbox" name="wn_android_statusbar[%s]" value="1" placeholder="" %s />',
      $args['id'],
